...
Dialog::Dialog(QWidget *parent)
    ...
    , waitResponseLabel(new QLabel(tr("Serial bitstream initialize")))
    , waitResponseSpinBox(new QSpinBox())
    , requestLabel(new QLabel(tr("Request:")))
    , requestLineEdit(new QLineEdit(tr("Response for bitstream trasmission")))
    , trafficLabel(new QLabel(tr("No traffic.")))
    , statusLabel(new QLabel(tr("Status: Not running.")))
    , runButton(new QPushButton(tr("Start")))
    , timeLabel(new QLabel(tr("Reconfiguration Time(ms):")))
{
    ...
    mainLayout->addWidget(timeLabel,4,0,1,4);
    mainLayout->addWidget(statusLabel, 5, 0, 1, 5);
    setLayout(mainLayout);

    setWindowTitle(tr("Ground Station Serial Bitstream Send"));
    ...
    connect(&thread, &MasterThread::time_rec,this,&Dialog::showTime);
}
...
void Dialog::transaction()
{
    ...
    //MasterThread* master = new MasterThread(this);
    //master->transaction(serialPortComboBox->currentText(),
                      // waitResponseSpinBox->value(),
                     //  requestLineEdit->text());
    //connect(master, &MasterThread::response, this, &Dialog::showResponse);
    //connect(master, &MasterThread::error, this, &Dialog::processError);
    //connect(master, &MasterThread::timeout, this, &Dialog::processTimeout);
    //connect(master, &MasterThread::finished, master, &MasterThread::deleteLater);
    //delete master;
...
void Dialog::showTime(const QString &s){
    setControlsEnabled(true);
    timeLabel->setText(tr("Reconfiguration Time(ms): %1").arg(s));
}
