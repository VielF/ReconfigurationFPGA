...
void main(){
   ...
   switch (restart_cause()){  //Relaiza verificação se houve reset por causa do WDT(ocorrência de falta)
    case WDT_TIMEOUT:           //ou se foi inicialização do uC
    {
      output_high(PIN_B0);      //Ativa interrupção externa no uC stepe
      delay_us(20);
      output_low(PIN_B0);
      WDT_RESET_FAULT = true;   //Identifica estouro do WDT
      break;
    }
    case NORMAL_POWER_UP:
	{
      WDT_RESET_FAULT = false;
      break;
    }
   }
...
/////////////////////////////////////////////////////////////////////////////////////
	...
	while(1){
      ...
      restart_wdt();  //reset do WDT para continuar execução normal
      if(WDT_RESET_FAULT == true){  //verifica se houve estouro do WDT, se sim entra em modo sleep
        setup_wdt(WDT_OFF);
		sleep();
      }else{
