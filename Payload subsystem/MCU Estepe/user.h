...
int1 WDT_RESET_FAULT = false;  //se houve falta (true) ou não (false)
int1 FPGA_REQ_RECONFIG = false; //inicia (true) ou não (false) a reconfiguração
int FPGA_PACKET_BLOCKS = 0; //controle para os dois pacotes de 64 bits que formam o pacote de 128 bits do AES, é zerado ao fim da descriptografia
int1 BUFFER_READY = false; // buffer que armazena os 256 bits de dados por página
...
//Vetor de dados de paginação
int DATA_PAGE[256] = {0x00}; //Armazena os dados descritografados a serem gravados em uma página
int DATA_PAGE_READ[256] = {0x00}; //Armazena os dados lidos da memoria

//endere�amento m�ximo da memoria 
int32 ADDR_MEM = 0;
//chave de descriptografia
const char DECODE_KEY[]= {0x13,0x11,0x1D,0x7F,0xE3,0x94,0x4A,0x17,0xF3,0x07,0xA7,0x8B,0x4D,0x2B,0x30,0xC5};
//Variavel de teste de página
